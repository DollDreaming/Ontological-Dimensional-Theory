...
저자 PC에서 하루 한 번, 이론 정리 문서가 바뀐 날에만 Claude Code가 위키를 고칩니다. 검사를 통과하면 사람 확인 없이 바로 게시됩니다(저자 결정, 2026-09-24).

- 검사: 기존 `docs/` 쪽만 고쳤나, `본편 v9.4` 인용 블록이 글자 그대로인가, 상자 밖 본문을 크게 지우지 않았나, 내부 이름·경로·연락처가 섞이지 않았나, `mkdocs build --strict`가 통과하나.
- 변경이 너무 크면 게시하지 않고 `claude/wiki-sync-hold` 브랜치에 둡니다. `main`과 비교해 보고 괜찮으면 풀 리퀘스트로 합치면 됩니다.
- 자동 갱신 커밋은 작성자가 `Claude (위키 자동 갱신)`으로 찍힙니다. 커밋 메시지에 바꾼 쪽과 근거가 적힙니다.
